    ...
    public class AttendanceHermesController : Controller
    {
        ...
        public JsonResult GetHermesValues(string date)
        {
            ...
            foreach (var item in groupes)
            {
                ...
                {
                    {
                        if (e.Id != empConnected.Id && e.Content != null && e.Roles.Any(r => r.UserId == e.Id && (r.RoleId == 3 || r.RoleId == 1009 || r.RoleId == 1010 || r.RoleId == 2013 || r.RoleId == 1011 || r.RoleId == 2016 || r.RoleId == 10 || r.RoleId == 2010)))
                        {
        ...
        public ActionResult SuiviTempsALLHermesUsers()
        {
            try
            {
                Employee empConnected = UserManager.FindById(Int32.Parse(User.Identity.GetUserId()));
        ...
        public JsonResult GetALLHermesValues(string date)
        {
            var attendances = db.attendancesHermes.ToList();
            DateTime dateSel = DateTime.Parse(date);
            ...
            List<Employee> liste = service.GetAll().ToList();
            foreach (var item in groupes)
            {
                List<Employee> emp = serviceGroupeEmp.getListEmployeeByGroupeId(item.Id);
                ...
                //foreach (var e in liste){
                //    if (e.Roles.Any(r => r.RoleId == 3 || r.RoleId == 1009 || r.RoleId == 1010 || r.RoleId == 2013 || r.RoleId == 1011 || r.RoleId == 2016 || r.RoleId == 10 || r.RoleId == 2010))
                //    {
                //        String strbase64 = Convert.ToBase64String(e.Content);
                //        Url = "data:" + e.ContentType + ";base64," + strbase64;
                //        ViewBag.url = Url;
